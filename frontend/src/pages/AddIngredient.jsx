...
const FloatingLabelInput = ({ name, value, onChange, placeholder, type = 'text' }) => (
    <div className="relative">
      <input
        ...
        placeholder=" " // The space is important for the peer selector to work
      />
      <label
        htmlFor={name}
...
// Reusable component for dynamic list items
...
// Component for the readable overview
      ...
      <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
        {isImage ? (
          ...
          <ul className="list-disc pl-5 space-y-1">
            {Object.entries(value).map(([key, val]) => val && val !== 'Select' && <li key={key}><span className="font-semibold capitalize">{key}:</span> {val}</li>)}
          </ul>
        ) : Array.isArray(value) ? (
          <ul className="list-disc pl-5 space-y-1">
            {value.map((item, index) => {
              if (typeof item === 'object') {
                ...
                return itemText && <li key={index}>{itemText}</li>;
              }
...
const AddIngredient = ({ navigateTo }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigateTo('ingredient-detail', response.data._id);
    } catch (error) {
        ...
        return (
          <div className="space-y-8">
            ...
            <div className="relative">
                ...
                <label
                    htmlFor="description"
                    className="absolute text-sm text-gray-500 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-green-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-6 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1"
                >
                    Ingredient Description *
                </label>
            </div>
            <input type="file" ref={fileInputRef} onChange={handleImageChange} className="hidden" accept="image/*" />
            <div className="w-full h-48 border-2 border-dashed rounded-lg flex flex-col items-center justify-center bg-gray-50 text-gray-500 cursor-pointer" onClick={() => fileInputRef.current.click()}>
              {imagePreview ? (
        ...
        return (
          <div className="space-y-8">
            <div>
              ...
              <button type="button" onClick={() => addDynamicListItem('whyToUse', '')} className="mt-3 text-sm text-green-600 font-semibold hover:text-green-700">Add Another Item</button>
            </div>
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {['vata', 'pitta', 'kapha'].map(type => (
                  <div key={type}>
                    ...
                    <select name={type} value={formData.prakritiImpact[type]} onChange={(e) => handleInputChange(e, 'prakritiImpact', type)} className="w-full p-3 border rounded-lg bg-white">
                      <option>Select</option><option>Balanced</option><option>Unbalanced</option>
                    </select>
                    <input value={formData.prakritiImpact[`${type}Reason`]} onChange={(e) => handleInputChange(e, 'prakritiImpact', `${type}Reason`)} placeholder={`${type} Reason`} className="mt-2 w-full p-3 border rounded-lg" />
                  </div>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="space-y-3">
                {formData.benefits.map((item, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    <button type="button" className="p-3 border rounded-lg"><Smile size={18} /></button>
                    <input value={item.text} onChange={(e) => handleComplexListChange(index, 'text', e.target.value, 'benefits')} placeholder="Benefit description" className="flex-grow p-3 border rounded-lg" />
              ...
              <button type="button" onClick={() => addDynamicListItem('benefits', { emoji: '', text: '' })} className="mt-3 text-sm text-green-600 font-semibold hover:text-green-700">Add Another Item</button>
            </div>
          </div>
        );
      case 3:
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <input value={formData.ayurvedicProperties.rasa} onChange={e => handleInputChange(e, 'ayurvedicProperties', 'rasa')} placeholder="Rasa *" className="p-3 border rounded-lg" />
                <input value={formData.ayurvedicProperties.veerya} onChange={e => handleInputChange(e, 'ayurvedicProperties', 'veerya')} placeholder="Veerya *" className="p-3 border rounded-lg" />
                <input value={formData.ayurvedicProperties.guna} onChange={e => handleInputChange(e, 'ayurvedicProperties', 'guna')} placeholder="Guna *" className="p-3 border rounded-lg" />
                <input value={formData.ayurvedicProperties.vipaka} onChange={e => handleInputChange(e, 'ayurvedicProperties', 'vipaka')} placeholder="Vipaka *" className="p-3 border rounded-lg" />
              </div>
            </div>
            <div>
              ...
              <div className="space-y-3">
                {formData.importantFormulations.map((item, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    ...
                    <input value={item.name} onChange={e => handleComplexListChange(index, 'name', e.target.value, 'importantFormulations')} placeholder="Formulation name" className="flex-grow p-3 border rounded-lg" />
              ...
              <button type="button" onClick={() => addDynamicListItem('importantFormulations', { name: '' })} className="mt-3 text-sm text-green-600 font-semibold hover:text-green-700">Add Another Item</button>
            </div>
            <div>
              ...
              <button type="button" onClick={() => addDynamicListItem('therapeuticUses', '')} className="mt-3 text-sm text-green-600 font-semibold hover:text-green-700">Add Another Item</button>
            </div>
          </div>
        );
      case 4:
              ...
              <div className="space-y-4">
                {formData.plantParts.map((item, index) => (
                  <div key={index} className="p-4 border rounded-lg bg-gray-50/50 space-y-3">
                    <div className="flex justify-end"><button type="button" onClick={() => removeDynamicListItem(index, 'plantParts')} className="p-1 text-gray-500 hover:text-red-600"><X size={18} /></button></div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <select value={item.part} onChange={e => handleComplexListChange(index, 'part', e.target.value, 'plantParts')} className="p-3 border rounded-lg bg-white">
                      ...
                      <input value={item.description} onChange={e => handleComplexListChange(index, 'description', e.target.value, 'plantParts')} placeholder="Description *" className="p-3 border rounded-lg" />
                    </div>
                  </div>
                ))}
              </div>
              <button type="button" onClick={() => addDynamicListItem('plantParts', { part: 'Select', description: '' })} className="mt-3 text-sm text-green-600 font-semibold hover:text-green-700">Add Another Item</button>
            </div>
            <div>
              <h3 className="text-lg font-medium text-gray-700 mb-3">Best Combined With *</h3>
              <input name="bestCombinedWith" value={formData.bestCombinedWith} onChange={handleInputChange} placeholder="Type here..." className="w-full p-3 border rounded-lg" />
            </div>
            <div>
              <h3 className="text-lg font-medium text-gray-700 mb-3">Geographical Locations *</h3>
              <input name="geographicalLocations" value={formData.geographicalLocations} onChange={handleInputChange} placeholder="Type here..." className="w-full p-3 border rounded-lg" />
            </div>
          </div>
        );
      case 5:
        return (
          <div className="space-y-4">
            ...
            <p className="text-gray-600">Review all the information below before submitting.</p>
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button type="button" onClick={() => navigateTo('ingredients-list')} className="p-2 text-gray-500 rounded-full hover:bg-gray-100">
          <X size={24} />
        </button>
      </div>
        ...
        {['General', 'Benefits', 'Properties', 'Other', 'Overview'].map((name, index) => (
          <React.Fragment key={index}>
            <div className="flex flex-col items-center">
              ...
              <p className={`mt-2 text-sm font-medium ${step >= index + 1 ? 'text-green-600' : 'text-gray-500'}`}>{name}</p>
            </div>
            {index < 4 && <div className={`flex-1 mx-4 h-0.5 transition-all duration-300 ${step > index + 1 ? 'bg-green-600' : 'bg-gray-300'}`}></div>}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className="flex justify-end space-x-4 mt-8">
        {step > 1 && <button type="button" onClick={prevStep} className="px-8 py-3 font-semibold border rounded-lg hover:bg-gray-100">Back</button>}
        {step < 5 && <button type="button" onClick={nextStep} className="px-8 py-3 font-semibold bg-green-700 text-white rounded-lg hover:bg-green-800">Next</button>}
        {step === 5 && <button type="button" onClick={handleSubmit} className="px-8 py-3 font-semibold bg-green-700 text-white rounded-lg hover:bg-green-800">Save</button>}
      </div>
    </div>
  );
};
